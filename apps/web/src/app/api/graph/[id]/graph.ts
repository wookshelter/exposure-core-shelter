...
export function resolveRootNode(
  ...
  const globalRoot = nodes.find(
    (n) => n.id.toLowerCase().includes(normalizedId) && n.chain === "global",
  );
  ...
  return (
    nodes.find((n) => n.id.toLowerCase().includes(normalizedId)) || nodes[0]
  );
}
...
export function calculateNodeContext(
  ...
  const totalIncomingUsd = incoming.reduce(
    (acc, e) => acc + e.allocationUsd,
    0,
  );
  const totalOutgoingUsd = outgoing.reduce(
    (acc, e) => acc + e.allocationUsd,
    0,
  );
